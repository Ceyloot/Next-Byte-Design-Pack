...
/**
 * MAPOWANIE ZMIENNYCH WYKRESU NA ZMIENNE MOTYWU — jedno na całą platformę.
 *
 * Wykres czyta 16 własnych zmiennych (`--chart-*`). Ktoś musi je połączyć
 * z motywem, bo inaczej są nieustawione i wykres renderuje się bez kolorów.
 * Do 02.08.2026 ta mapa stała INLINE w `management/TokensAnalytics` — czyli
 * każdy następny ekran z wykresem musiałby ją przepisać, a pierwsza literówka
 * przy przepisywaniu dawałaby wykres w innych barwach niż sąsiedni.
 *
 * Użycie: `<div style={ZMIENNE_WYKRESU}> <AreaChart …/> </div>`
 */
export const ZMIENNE_WYKRESU = {
  '--chart-background': 'transparent',
  '--chart-foreground': 'hsl(var(--foreground))',
  '--chart-foreground-muted': 'hsl(var(--muted-foreground))',
  '--chart-label': 'hsl(var(--muted-foreground))',
  '--chart-line-primary': 'hsl(var(--primary))',
  '--chart-line-secondary': 'hsl(var(--muted-foreground) / 0.5)',
  '--chart-crosshair': 'hsl(var(--muted-foreground) / 0.4)',
  '--chart-grid': 'hsl(var(--border) / 0.3)',
  '--chart-indicator-color': 'hsl(var(--primary))',
  '--chart-indicator-secondary-color': 'hsl(var(--muted-foreground))',
  '--chart-marker-background': 'hsl(var(--card))',
  '--chart-marker-border': 'hsl(var(--border))',
  '--chart-marker-foreground': 'hsl(var(--foreground))',
  '--chart-marker-badge-background': 'hsl(var(--card))',
  '--chart-marker-badge-foreground': 'hsl(var(--foreground))',
...
function DateTicker({ currentIndex, labels, visible }: DateTickerProps) {
  ...
  return (
    <motion.div
      className="overflow-hidden rounded-full bg-card px-4 py-1 text-foreground shadow-lg"
      layout
      transition={{
  ...
  return (
    ...
    >
      <div className="px-3 py-2.5" ref={measureRef}>
        {title && (
          <div className="mb-2 font-medium text-xs text-muted-foreground">
            {title}
          </div>
        )}
...
function TooltipBox({
  ...
  /*
    WSPÓŁRZĘDNE VIEWPORTU, NIE KONTENERA (06.08.2026, zgłoszenie Michała
    z Przeglądu Zarządu: dymek „chowa się pod kartą").

    Dymek portalował się do kontenera wykresu WEWNĄTRZ karty. Skutki, oba
    zmierzone w diagnozie:
      • karta ma `overflow-hidden`, a mini-wykres w KartaMetryki ~40 px
        wysokości przy dymku ~80 px — cięcie na krawędzi karty było
        gwarantowane z geometrii;
      • `backdrop-filter` karty tworzy stacking context, więc z-50 dymka
        obowiązywał tylko WEWNĄTRZ niej — sąsiednia karta malowała się nad nim.

    Teraz portal idzie do `document.body`, a pozycja liczy się od
    `getBoundingClientRect()` kontenera — `position: fixed` i rect mówią tym
    samym układem współrzędnych, także przy przewiniętej stronie. Zaciskanie
    i odbicie liczone od viewportu, nie od wnętrza karty.
  */
  const rect = containerRef.current?.getBoundingClientRect();
  const baseLeft = rect?.left ?? 0;
  const baseTop = rect?.top ?? 0;

  const shouldFlipX = baseLeft + x + tooltipWidth + offset > window.innerWidth;
  const targetX = baseLeft + (shouldFlipX ? x - offset - tooltipWidth : x + offset);

  const targetY = Math.max(
    offset,
    Math.min(baseTop + y - tooltipHeight / 2, window.innerHeight - tooltipHeight - offset)
  );
  ...
  /*
    Nadpisania (`left`/`top`) przychodzą z ChartTooltip we współrzędnych
    KONTENERA — dla wykresów pionowych `top={margin.top}`. Skoro dymek stoi
    teraz `fixed`, trzeba je przesunąć o pozycję kontenera, inaczej przykleiłby
    się do górnej krawędzi ekranu.
  */
  const finalLeft = typeof leftOverride === 'number' ? baseLeft + leftOverride : (leftOverride ?? animatedLeft);
  const finalTop = typeof topOverride === 'number' ? baseTop + topOverride : (topOverride ?? animatedTop);
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        /*
          MATERIAŁ DYMKA — `.nb-szklo`, nie `bg-popover`.

          Było: `bg-popover text-popover-foreground shadow-lg`.
          `nb-szklo` bez przezroczystego wypełnienia nic nie robi, bo
          `--popover` jest NIEPRZEZROCZYSTY — rozmycie nie ma czego rozmyć.
          Dokładnie ta sama pułapka co w `Tile` (stary przepis obok nowego):
          w kodzie widać „blur", a na ekranie płaską płytę. Michał wyłapał to
          na wykresie: „czemu nie ma to glassmorphizmu".

          `.nb-szklo` niesie komplet: wypełnienie zależne od motywu, nasycenie,
          rozmycie, rant i cień. Bez `nb-szklo-plynne` — załamanie na dymku
          wielkości znaczka pocztowego kosztuje tyle samo co na oknie, a nie
          ma go gdzie zobaczyć.
        */
        className="min-w-[140px] overflow-hidden rounded-lg text-card-foreground nb-szklo nb-szklo-plynne nb-szklo-tafla"
    ...
    /*
      PORTAL DO `document.body`, NIE DO KONTENERA WYKRESU (06.08.2026).

      Dymek był potomkiem karty, więc: (1) `overflow-hidden` karty go ucinał —
      mini-wykres w KartaMetryki ma ~40 px, dymek ~80 px, cięcie wynikało
      z geometrii; (2) `backdrop-filter` karty tworzy stacking context, więc
      z-50 działał tylko WEWNĄTRZ niej i sąsiednia karta malowała się nad nim;
      (3) jako szyba w szybie łapał regułę zagnieżdżenia i tracił rozmycie —
      stąd „półprzezroczysta ciemna płyta".

      Na `document.body` wszystkie trzy znikają naraz: nic go nie tnie, nic nie
      maluje się nad nim, a materiał próbkuje realną stronę.
    */
    document.body
  );
}
...
export function ChartTooltip({
  /*
    PLAKIETKA DATY DOMYŚLNIE WYŁĄCZONA.

    Michał: „czemu pokazujesz to czarne okienko z datą która się dubluje z tym
    większym, nie mieliśmy tego ogarnąć?" — miał rację podwójnie. Po pierwsze
    plakietka jest ZAWSZE duplikatem: renderuje się wyłącznie razem z dymkiem
    (warunek `visible`), a dymek zawsze niesie datę w nagłówku. Po drugie
    naprawiłem to wcześniej TYLKO w karcie metryki (`showDatePill={false}`
    w `wskazniki.tsx`) i zameldowałem jako zrobione, podczas gdy pozostałe
    wykresy zostały nietknięte.

    Domyślna wartość, a nie prop przy każdym użyciu: sześć plików z wykresami
    to sześć okazji, żeby jedno przeoczyć — dokładnie tak, jak przeoczyłem
    za pierwszym razem. Kto naprawdę chce plakietkę na osi, poda `showDatePill`.
  */
  showDatePill = false,
  showCrosshair = true,
  showDots = true,
  ...
  const {
    ...
    orientation,
    barXAccessor,
    formatDateLabel: ctxFormatDateLabel,
  } = useChart();
  ...
  const title = useMemo(() => {
    ...
    if (ctxFormatDateLabel) {
      return ctxFormatDateLabel(date);
    }
  ...
  }, [tooltipData, barXAccessor, xAccessor, ctxFormatDateLabel]);
...
export function XAxis({ numTicks = 5, tickerHalfWidth = 50, formatLabel, tickValues }: XAxisProps) {
  const { data, xAccessor, xScale, margin, tooltipData, containerRef } = useChart();
  const [mounted, setMounted] = useState(false);
  ...
  const labelsToShow = useMemo(() => {
    ...
    const tickCount = Math.max(1, Math.min(numTicks, data.length));
    const indices =
    ...
    return indices.map((index) => {
      const date = xAccessor(data[index]);
      return {
        date,
        x: (xScale(date) ?? 0) + margin.left,
        label: formatLabel
  ...
  }, [data, xAccessor, xScale, margin.left, numTicks, formatLabel, tickValues]);
...
function ChartInner({
  ...
  const yScale = useMemo(() => {
    ...
    // If data spans both positive and negative, make domain symmetric around 0
    if (minValue < 0 && maxValue > 0) {
      const absMax = Math.max(Math.abs(minValue), Math.abs(maxValue)) * 1.15;
      return scaleLinear({
        range: [innerHeight, 0],
    ...
    const domainMin = minValue < 0 ? minValue * 1.15 : 0;
  ...
  const dateLabels = useMemo(
    () =>
      data.map((d) => {
        ...
        if (formatDateLabel) return formatDateLabel(date);
        return date.toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        });
      }),
    [data, xAccessor, formatDateLabel]
  );
