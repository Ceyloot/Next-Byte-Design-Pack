...
// Physics constants for MD3 ripple
...
interface RippleProps extends React.HTMLAttributes<HTMLDivElement> {
  /**
   * Tailwind text color class for the ripple (e.g., "text-foreground", "text-blue-500").
...
const Ripple = forwardRef<HTMLDivElement, RippleProps>(
  (
    ...
    const stateRef = useRef<RippleState>(RippleState.INACTIVE);
    ...
    return (
      ...
      >
        ...
        <div className={cn("absolute inset-0 pointer-events-none", color)}>
          {/* Hover Layer */}
          <div
            className="absolute inset-0 bg-current transition-opacity duration-200"
            style={{ opacity: isHovered && !disabled ? opacity * 0.5 : 0 }}
          />
          ...
          <div
            ...
            style={{
              ...
              transition: isPressed ? "none" : "opacity 300ms ease-out",
            }}
          />
        </div>
      </div>
